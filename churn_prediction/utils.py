...
def _get_ml_config_value(env, key):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"churn_prediction have been deployed to {env} (see churn_prediction"
...
def get_model_name(env, test_mode=False):
    ...
    if env == "dev" or test_mode:
        ...
        return f"churn_prediction-model{resource_name_suffix}"
    else:
        # Read ml model name from churn_prediction/terraform
        return _get_ml_config_value(env, "churn_prediction_model_name")
