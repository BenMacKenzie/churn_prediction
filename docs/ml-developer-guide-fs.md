...
* Python unit tests for feature computation modules in `churn_prediction/tests/feature_engineering` folder.
...
1. Implement your feature module, address TODOs in `churn_prediction/feature_engineering/features` and create unit test in `churn_prediction/tests/feature_engineering`
2. Update `churn_prediction/terraform/staging/write-feature-table-job.tf` and `churn_prediction/terraform/prod/write-feature-table-job.tf`. Fill in parameters for `GenerateAndWriteFeatures` job.
3. Update training data path in `churn_prediction/terraform/staging/training-job.tf` and `churn_prediction/terraform/prod/training-job.tf`
...
* Run `databricks configure --profile churn_prediction-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile churn_prediction-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)
