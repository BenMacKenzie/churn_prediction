resource "databricks_job" "model_training_job" {
  name = "${local.env_prefix}churn_prediction-model-training-job"
  ...
  task {
    ...
    notebook_task {
      notebook_path = "churn_prediction/training/notebooks/TrainWithFeatureStore"
      base_parameters = {
        env = local.env
        ...
        model_name         = "${local.env_prefix}churn_prediction-model"
      }
    }
  ...
  task {
    ...
    notebook_task {
      notebook_path = "churn_prediction/validation/notebooks/ModelValidation"
      base_parameters = {
        experiment_name = databricks_mlflow_experiment.experiment.name
        ...
        # Specifies the name of the function in churn_prediction/validation/validation.py that returns custom metrics.
        # TODO(optional): custom_metrics_loader_function
        custom_metrics_loader_function = "custom_metrics"
        # Specifies the name of the function in churn_prediction/validation/validation.py that returns model validation thresholds.
        # TODO(optional): validation_thresholds_loader_function
        validation_thresholds_loader_function = "validation_thresholds"
        # Specifies the name of the function in churn_prediction/validation/validation.py that returns evaluator_config.
  ...
  task {
    ...
    notebook_task {
      notebook_path = "churn_prediction/deployment/model_deployment/notebooks/TriggerModelDeploy"
      base_parameters = {
        env = local.env
      }
    }
